...
const Sidebar = ({ isOpen }: SidebarProps) => {
  ...
  return (
    <div className={`w-[260px] h-screen bg-[#0f172a] flex flex-col fixed left-0 top-0 z-50 border-r border-slate-800 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      {/* Logo */}
      ...
      <div className="flex-1 py-4 overflow-y-auto custom-scrollbar">
        ...
        <SidebarItem
          ...
          subItems={[
            ...
            { label: 'Quyền hạn', to: '/admin/roles' },
            { label: 'Phân quyền', to: '/admin/permissions' },
          ]}
        />
        ...
        <SidebarItem icon={CalendarToday} label="Lịch trình" to="/admin/schedules" />
        
        <SidebarItem icon={Circle} label="Trạng thái ghế" to="/admin/seat-status" /> 
        {/* Using Circle as placeholder for Seat Status icon if specific one not clear */}

        <SidebarItem icon={ConfirmationNumber} label="Vé & Hành khách" to="/admin/tickets" />
        ...
        <SidebarItem icon={Description} label="Chính sách hủy" to="/admin/cancellation-policies" />
        
        <SidebarItem icon={Star} label="Đánh giá" to="/admin/reviews" />
        
        <SidebarItem icon={Image} label="Banner" to="/admin/banners" />
