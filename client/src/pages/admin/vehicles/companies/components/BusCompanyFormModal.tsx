...
export default function BusCompanyFormModal({ open, onClose, onSubmit, initialData }: Props) {
  ...
  const handleThumbnailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // In a real app, you'd upload here. For now we just preview.
      // We also update form.image to the objectUrl so it feels "live", 
      // though typically this would be a server URL.
  ...
  const handleSubmit = async () => {
    ...
    if (!hasError) {
      try {
        ...
        if (thumbnailFile) {
           imageUrl = await uploadImageToCloudinary(thumbnailFile);
        }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        className="
          ...
          [@media(min-width:769px)]:max-w-[650px]
      ...
      >
        ...
        <div className="px-4 py-4 space-y-6 text-sm">
          ...
          <div className="space-y-4">
            <SectionTitle text="Thông tin nhà xe" />
            ...
            <div className="grid grid-cols-1 gap-3 [@media(min-width:391px)]:grid-cols-2">
              ...
              {/* STATUS */}
              <div className="[@media(min-width:391px)]:col-span-2">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <label className="inline-block px-4 py-2 bg-blue-50 text-blue-600 rounded cursor-pointer hover:bg-blue-100 transition text-sm font-medium">
                 ...
                 {/* Optional: Input for manual URL to support db.json persistence */}
                 <div className="mt-2">
                    <input 
                        type="text" 
                        ...
                        className="text-xs border border-gray-300 rounded px-2 py-1 w-full max-w-[200px]"
                    />
                 </div>
                <p className="text-xs text-gray-500 mt-2">
                  Chọn ảnh đại diện cho nhà xe.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex justify-end gap-2 px-4 py-3 border-t border-gray-200">
          <button
            onClick={onClose}
            disabled={uploading}
            className="px-4 py-2 border border-gray-300 rounded text-sm hover:cursor-pointer disabled:opacity-50"
          >
            Hủy
          </button>
          <button
            ...
            disabled={uploading}
            className="px-4 py-2 bg-blue-600 text-white rounded text-sm hover:cursor-pointer disabled:opacity-50"
          >
...
function SectionTitle({ text }: { text: string }) {
  return (
    <h3 className="font-medium text-gray-700 text-sm border-l-4 border-blue-500 pl-2">
      {text}
    </h3>
  );
}
